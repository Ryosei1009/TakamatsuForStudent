    ...
    return (
        <>
            <Modal
                ...
                className={`transition-opacity w-full max-w-240 max-lg:max-w-23/24 bg-gray-100 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute pt-8 max-sm:pt-6 pb-5 px-16 max-sm:px-4 rounded-xl outline-none`}
            >
                {parseInt(eachAccount.id) === selectedPhoto.created_by_id || parseInt(eachAccount.role) <= 2 ? (
                    <div className="absolute right-6">
                        ...
                        {showPopup && (
                            <div className="top-8 -right-3 cursor-pointer hover:underline hover:opacity-90 absolute z-10 bg-white border rounded text-center shadow-sm py-2 px-4 w-18 text-red-500 font-bold" onClick={() => deleteData(`/delete/photos/${selectedPhoto.id}`, '')}>
                                削除
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        />
                            ...
                            <img
                                src={`${process.env.REACT_APP_IMAGE_DOMAIN}/${selectedPhoto.image_name}`}
                                alt=""
                                className="h-auto max-h-120 w-auto"
                            />
                        )}
                    </div>
